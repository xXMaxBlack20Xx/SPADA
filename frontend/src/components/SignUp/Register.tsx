...
export default function Register() {
    ...
    return (
        <section className="relative isolate min-h-screen grid place-items-center overflow-hidden bg-linear-to-b from-[#0B0A17] via-[#121024] to-[#1A132B] text-(--color-text) p-6 md:p-10">
            {/* Card */}
            <main className="relative z-10 w-full sm:max-w-md md:max-w-lg lg:max-w-xl rounded-2xl border backdrop-blur-xl bg-[color-mix(in_oklab,var(--color-surface)_68%,transparent)] border-[color-mix(in_oklab,var(--color-text-muted)_22%,transparent)] shadow-[0_10px_40px_-15px_color-mix(in_oklab,var(--color-primary)_55%,transparent)] px-6 py-7 md:px-10 md:py-10 mt-20">
                <button
                    onClick={() => navigate('/')}
                ...
                <form className="grid gap-5" onSubmit={onSubmit}>
                    ...
                    <label className="flex items-start gap-3 text-sm select-none">
                        ...
                        />
                        <span>
                            {t.helper} <a href="#">{t.termsA}</a> & <a href="#">{t.termsB}</a>.
                        </span>
                    </label>
